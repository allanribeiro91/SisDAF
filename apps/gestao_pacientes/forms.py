...
class Atendimento_JudicialForm(forms.ModelForm):
    data_solicitacao  = forms.DateField(
        widget=forms.DateInput(attrs={
            'class': 'form-control',
            'type': 'date',
    ...
    origem_solicitacao = forms.ChoiceField(
        choices=ORIGEM_DEMANDA_JUDICIAL_SOLICITACAO,
        widget=forms.Select(attrs={
            'class': 'form-select',
            'id':'id_atendjudicial_origem_solicitacao'
        }),
        label='Origem da Solicitação',
        initial='',
        required=True,
    )
    meio_solicitacao -> charfield choices
    uf_origem -> charfield choices LISTA_UFS_SIGLAS
    acao_civil_publica -> charfield choices SIM_NAO
    documento_sei_comprovante -> charfield
    processo_sei_comprovante -> charfield
    produto -> foreingkey
